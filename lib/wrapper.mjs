...
import { sanitizeTimestamp, clampAgentMs } from './sanitize.mjs';
...
function verifyHookTimestamps() {
  ...
  // Resolve hooks relative to this module, not a hardcoded ~/Documents/GitHub
  // path — the repo now lives at ~/git/claude_timings_wrapper (and could live
  // anywhere). wrapper.mjs is in lib/, hooks are in ../hooks.
  const hooksDir = join(dirname(fileURLToPath(import.meta.url)), '..', 'hooks');
  const startScript = join(hooksDir, 'claude-timing-start.sh');
  const stopScript = join(hooksDir, 'claude-timing-stop.sh');
...
export function startWrapper(claudeArgs) {
  ...
  // Incremental git scan of this repo (background, non-blocking)
  // Catches any commits made in this repo while we were away.
  try {
    const scanScript = join(homedir(), '.claude', 'hooks', 'scan_git.sh');
    if (existsSync(scanScript)) {
      spawn('bash', [scanScript, sessionCwd], {
        detached: true,
        stdio: 'ignore',
      }).unref();
    }
  ...
  const sessionStartMs = Date.now();  // floor for hook-timestamp plausibility checks
  let phaseStart = sessionStartMs;
  let promptCount = 0;
  ...
  const child = pty.spawn('claude', claudeArgs, {
    ...
    env: {
      ...
      CLAUDE_TIMING_LOG: sessionFilePath,
    },
  });
  ...
  function handleStopHook() {
    ...
    try {
      ...
      // Reject a corrupt/partial-read epoch (near-zero or future); fall back to
      // wall-clock so a bad file can't produce a decades-long phase.
      stopTimestamp = sanitizeTimestamp(stopTimestamp, sessionStartMs, Date.now()) ?? Date.now();
      unlinkSync(tmpFile);
    } catch {
      return;
    }
    ...
    if (state === STATE.AGENT_WORKING) {
      const { ms: agentMs, clamped } = clampAgentMs(stopTimestamp - phaseStart);
      ...
      const { ms: backgroundMs, clamped } = clampAgentMs(stopTimestamp - phaseStart);
  ...
  function handleStartHook() {
    ...
    try {
      ...
      // Reject a corrupt/partial-read epoch (near-zero or future); fall back to
      // wall-clock so a bad phaseStart can't yield a 55-year agent turn.
      startTimestamp = sanitizeTimestamp(startTimestamp, sessionStartMs, Date.now()) ?? Date.now();
  ...
  process.stdin.on('data', (data) => {
    ...
    if (buf.length === 1 && buf[0] === 0x03) {
      if (state === STATE.AGENT_WORKING) {
        const { ms: agentMs, clamped } = clampAgentMs(Date.now() - phaseStart);
        totalAgentMs += agentMs;
        promptCount++;
  ...
  async function cleanup(exitCode) {
    ...
    if (state === STATE.AGENT_WORKING) {
      const { ms: partialAgent } = clampAgentMs(Date.now() - phaseStart);
